...
* 调用与执行分离, 匹配二者不同的处理速度
* `Request`可以继承构成类树利用多态机制

# Future

##  形式

请求者发出请求之后立即得到一张订货单,之后工厂生产货物,请求者凭借订货单来在之后获取货物

## 角色

1. `Main`类 : 负责发送请求,立即可以获得一个凭证,在一段时间之后可以由凭证取得真正的数据,在这段时间之间,主线程(即顾客)可以去从事其他工作

2. `Host`类 : 在主类发出请求之后进行如下工作 :

   1. 立即创建一个凭证
   2. 创建并启动一个线程去处理获得真正的数据并填入对应凭证的数据域
   3. 将凭证返回给请求的调用者

   **以上三步基本不耗费时间,真正花时间的处理创建的线程去处理真正的数据**

3. `Data`类 : 分为真正的数据和凭证数据

   ![](image/Future-design-uml.png)

![](image/Future-design-seq.png)
...
即 : 正在进行操作的线程 ---> 收到终止命令 ---> 进行终止之前的一系列处理 ---> 安全的终止线程

![](image/set_end.png)

## 实现

将终止处理程序放到中断异常捕捉处理程序中

当接收到终止信号之后设置标记位并产生**中断**

## 补充

关于`interrupt`方法 : 

调用该方法之后根据线程当前状态的不同有两种情况 :

1. 状态 : 一般而言线程状态变为中断状态
2. 控制 : 当线程在`sleep, wait, join`中时抛出中断异常
...
1. 中断状态抛出中断异常
   ...
   ```java
   if (Thread.interrupted()) {	//检测的if语句所在的线程的状态
       throws new InterruptionExecption();
   }
   ```
